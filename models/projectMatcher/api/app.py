...
def recommend_teams(project_skills, teams_df, top_n=5, min_similarity=0.1):
    ...
    try:
        ...
        project_vector = vectorizer.transform([project_skills])
        similarities = cosine_similarity(project_vector, team_vectors).flatten()
...
def ml_recommend():
    ...
    recommendations, total_teams, teams_above_threshold = recommend_teams(
        project_skills, teams_df, top_n, min_similarity
    )
